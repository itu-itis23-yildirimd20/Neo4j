# Neo4j: Practical Demo & Query Guide

This repository provides a step-by-step walkthrough for running a practical Neo4j demo using the AuraDB cloud platform and the built-in "Movie Dataset".

**Created by: Defne Yıldırım (150230727)**

---

## 🗂️ Project Files

* **[Neo4j.pdf](Neo4j.pdf):** The PDF presentation slides.
* **[queries.cypher](queries.cypher):** A standalone file containing all demo Cypher queries.

---

## 🚀 How to Replicate This Demo (Step-by-Step)

This guide is designed to be replicated by anyone in under 10 minutes. **No local installation is required.**

### 1. Sign Up for Neo4j Aura (The "Tool")
* Go to the [Neo4j Aura](https://neo4j.com/cloud/aura-free/) website.
* Sign up for a new account (a Google/GitHub/Apple account works).

### 2. Create the Database Instance
* First, [Sign up for a free Neo4j Aura account](https://neo4j.com/cloud/aura-free/).
* From your dashboard, go to the **Instances** section and click **"Create Instance"**.
* Select the **"AuraDB Free"** option.
* Set an instance name (e.g., "Instance01") and a password, then wait 2-3 minutes for it to be created.
...
* When that guide is finished, you can close it. You will now have data (Nodes & Relationships) in your database.

### 4. Run the Demo Queries
* Now that your database is loaded with data, you can run the demo queries.
* Copy the queries from the `Demo Queries (Cypher)` section below (or from the `queries.cypher` file).
* Paste them one-by-one into the top query bar (where it says `neo4j$`).
* Press the **"Play" (Run)** button for each query to see the live, visual results.

## 📸 Demo Screenshot

Here is an example of the visual result from running Query 4 (the shortest path between Tom Hanks and Kevin Bacon).

![Neo4j Demo Graph](visualisation.jpg)

---

## 💻 Demo Queries (Cypher)

Use these queries in the Neo4j Browser to explore the Movie Dataset.

#### Query 1: Find a specific entity
Finds the Person node with the name "Tom Hanks".
```cypher
...
Finds all movies released in the 1990s.
...
#### Query 4: Find the shortest path

Finds the shortest path (relationship degrees) between Tom Hanks and Kevin Bacon.

```cypher
MATCH path = shortestPath(
  (p1:Person {name: "Tom Hanks"})-[*]-(p2:Person {name: "Kevin Bacon"})
)
RETURN path
```
